...
const RandomizerWidget: React.FC<WidgetProps> = ({ widget, updateData }) => {
  const { students = [], currentName, isAnimating, fontSize = 16 } = widget.data;
  const [showSettings, setShowSettings] = useState(students.length === 0);
  ...
  }, [isAnimating, widget.id, students, updateData]);
    ...
    return (
      <div className="flex flex-col h-full bg-white text-sm">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
          ...
          {students.length === 0 && <div className="text-gray-400 text-center py-4">No students added</div>}
        </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-4 bg-indigo-50 relative group">
      ...
      <button 
        ...
        disabled={isAnimating || students.filter(s => s.active).length === 0} 
