...
import { Type, Grid, RefreshCw, Settings, X } from 'lucide-react';
...
export const LexiGuess = ({ widget, updateData }) => {
    ...
    const resetGame = () => {
        const words = WORD_BANKS[wordLength];
    ...
    const handleKey = (key: string) => {
        ...
        if (key === 'Enter') {
            if (input.length === wordLength) {
                ...
                let status = 'playing';
                if (input.toUpperCase() === target) status = 'won';
    ...
    }, [input, widget.data.wordleTarget, isGameOver, wordLength]);
        ...
        <div className="p-4 h-full flex flex-col items-center justify-center gap-4 bg-slate-100/50">
            ...
            {isGameOver && (
                <div className="text-center animate-in fade-in zoom-in slide-in-from-bottom-4">
                    <div className={`text-lg font-black mb-2 ${widget.data.wordleStatus === 'won' ? 'text-green-600' : 'text-red-500'}`}>
...
export const GridGlide = ({ widget, updateData }) => {
    ...
    const handlePointerUp = () => {
        ...
        if (foundWords.includes(word)) {
            ...
            const bank = WORD_BANKS[word.length as 4 | 5 | 6] || [];
            if (bank.includes(word) || (word.length >= 3 && bank.length === 0)) {
    ...
    return (
        <div className="p-4 h-full flex flex-col items-center gap-4 bg-slate-100/50 overflow-y-auto" onPointerUp={handlePointerUp}>
            <div className="flex justify-between w-full max-w-[280px] mb-2">
                ...
                {message && <div className="text-[10px] font-black uppercase text-blue-600 animate-pulse">{message}</div>}
            </div>
...
export const ScrambleSwap = ({ widget, updateData }) => {
    ...
    const isSolved = current.length > 0 && current.join('') === target;
    ...
    const resetGame = () => {
        const words = WORD_BANKS[wordLength as 4 | 5 | 6];
        const newTarget = words[Math.floor(Math.random() * words.length)];
        updateData(widget.id, {
    ...
    const swap = (i: number, j: number) => {
        ...
        updateData(widget.id, { scrambleCurrent: next });
        setSelectedIndex(null);
    };
...
const GamesWidget = ({ widget, updateData }) => {
